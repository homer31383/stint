...
import { useOfflineFirst, useOfflineSettings } from "./hooks/useOfflineFirst";
...
// STORAGE
// ============================================================
const load = (k, fb) => { try { const d = localStorage.getItem("stint_" + k); return d ? JSON.parse(d) : fb; } catch { return fb; } };
const save = (k, d) => localStorage.setItem("stint_" + k, JSON.stringify(d));
  ...
  return (
    <div style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        borderRadius: mob ? "14px 14px 0 0" : "14px",
        width: mob ? "100%" : `min(${width}px, 94vw)`,
        maxHeight: mob ? "92vh" : "88vh", overflow: "auto",
        padding: mob ? "20px 16px" : "24px",
        paddingBottom: mob ? "calc(16px + env(safe-area-inset-bottom, 0px))" : "24px",
        boxShadow: "0 20px 60px rgba(0,0,0,0.12), 0 4px 16px rgba(0,0,0,0.06)",
      }}>
        {mob && <div style={{ width: "36px", height: "4px", borderRadius: "2px", background: t.border, margin: "0 auto 16px" }} />}
...
export default function App() {
  ...
  const [clients, setClients] = useOfflineFirst("clients", []);
  const [projects, setProjects] = useOfflineFirst("projects", []);
  ...
  const handleAuth = async () => {
    ...
    else if (authMode === "signup") setAuthError("Check your email for a confirmation link.");
  };

  useEffect(() => {
    const mq = window.matchMedia("(max-width: 767px)");
    const handler = (e) => { setIsMobile(e.matches); if (!e.matches) setMoreMenuOpen(false); };
  ...
  // One-time cleanup: deduplicate Internal Meeting projects per client
  const didCleanup = useRef(false);
  useEffect(() => {
    if (didCleanup.current || clients.length === 0) return;
    didCleanup.current = true;
    setProjects(prev => {
      // Group Internal Meeting projects by clientId
      const byClient = {};
      prev.forEach(p => {
      ...
      for (const dupes of Object.values(byClient)) {
        if (dupes.length <= 1) continue;
        // Count references: time entries + pencils pointing to each project
        const refCount = (id) =>
          timeEntries.filter(e => e.projectId === id).length +
          pencils.filter(b => b.projectId === id).length;
        // Sort: most references first, then oldest createdAt as tiebreaker
        dupes.sort((a, b) => refCount(b.id) - refCount(a.id) || (a.createdAt || 0) - (b.createdAt || 0));
        // Keep first (most referenced / oldest), mark rest for removal only if zero references
        for (let i = 1; i < dupes.length; i++) {
          if (refCount(dupes[i].id) === 0) dupeIds.add(dupes[i].id);
        }
      }
      if (dupeIds.size === 0) return prev;
      return prev.filter(p => !dupeIds.has(p.id));
    });
  }, [clients, timeEntries, pencils]);

  // Ensure every client has an Internal Meeting project
  useEffect(() => {
    if (clients.length === 0) return;
    setProjects(prev => {
      const missing = clients.filter(c => c.id !== PERSONAL_CLIENT_ID && !prev.some(p => p.clientId === c.id && p.name === "Internal Meeting"));
      if (missing.length === 0) return prev;
      return [...prev, ...missing.map(c => ({ id: uid(), clientId: c.id, name: "Internal Meeting", notes: "", createdAt: Date.now() }))];
    });
  }, [clients]);
  ...
  if (authLoading) return (
    <div style={{ fontFamily: "'Instrument Sans', sans-serif", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", color: t.textSecondary }}>
  ...
  if (supabase && !session) return (
    <div style={{ fontFamily: "'Instrument Sans', sans-serif", background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh" }}>
      <link href="https://fonts.googleapis.com/css2?family=Instrument+Sans:wght@400;500;550;600;700&display=swap" rel="stylesheet" />
      <div style={{ background: t.white, border: `1px solid ${t.border}`, borderRadius: "14px", padding: "36px", width: "100%", maxWidth: "360px", boxShadow: "0 4px 16px rgba(0,0,0,0.06)" }}>
        <h1 style={{ fontSize: "22px", fontWeight: 750, letterSpacing: "-0.03em", margin: "0 0 4px", color: t.text }}>Stint</h1>
        <p style={{ fontSize: "13px", color: t.textSecondary, margin: "0 0 24px" }}>Sign in to continue</p>
        <Field label="Email" type="email" value={authEmail} onChange={setAuthEmail} placeholder="you@example.com" />
        <div style={{ height: "10px" }} />
        <Field label="Password" type="password" value={authPassword} onChange={setAuthPassword} placeholder="Your password" />
        <div style={{ height: "18px" }} />
        <button onClick={handleAuth} style={{
          width: "100%", padding: "10px", border: "none", borderRadius: "7px", cursor: "pointer",
          background: t.black, color: t.white, fontSize: "13px", fontWeight: 650,
          fontFamily: "'Instrument Sans', sans-serif", letterSpacing: "-0.01em",
        }}
          onMouseEnter={e => e.currentTarget.style.opacity = "0.85"}
          onMouseLeave={e => e.currentTarget.style.opacity = "1"}
        >{authMode === "login" ? "Sign In" : "Create Account"}</button>
        {authError && <p style={{ fontSize: "12px", color: authError.startsWith("Check") ? t.green : t.red, margin: "12px 0 0", textAlign: "center" }}>{authError}</p>}
        <p style={{ fontSize: "12px", color: t.textTertiary, textAlign: "center", margin: "16px 0 0" }}>
  ...
  return (
    <div style={{ fontFamily: "'Instrument Sans', sans-serif", background: t.bg, color: t.text, minHeight: "100vh", display: "flex", flexDirection: "column" }}>
      ...
      <header style={{
        ...
        padding: isMobile ? "0 16px" : "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between",
        height: "52px", flexShrink: 0,
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
          <span style={{ fontSize: "15px", fontWeight: 750, color: t.text, letterSpacing: "-0.03em" }}>Stint</span>
          {!isMobile && <nav style={{ display: "flex", gap: "2px" }}>
            {navItems.map(n => (
      ...
      {isMobile && (
        <nav style={{
          ...
          {[
            ...
            { id: "time", label: "Time", icon: "M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" },
            { id: "pencils", label: "Bookings", icon: "M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" },
            ...
            return (
              <button key={item.id} onClick={() => { setTab(item.id); setMoreMenuOpen(false); }}
                style={{
                  flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                  gap: "2px", background: "transparent", border: "none", cursor: "pointer",
                  color: active ? t.green : t.textTertiary, padding: "4px 0",
                ...
                <span style={{ fontSize: "10px", fontWeight: active ? 700 : 500, letterSpacing: "0.01em" }}>{item.label}</span>
              </button>
            );
          })}
          <button onClick={() => setMoreMenuOpen(v => !v)}
            style={{
              flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
              gap: "2px", background: "transparent", border: "none", cursor: "pointer",
              color: ["clients", "reports", "settings"].includes(tab) ? t.green : t.textTertiary, padding: "4px 0",
              fontFamily: "'Instrument Sans', sans-serif",
            }}>
            <svg width="22" height="22" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24">
              <circle cx="12" cy="12" r="1" /><circle cx="12" cy="5" r="1" /><circle cx="12" cy="19" r="1" />
            </svg>
            <span style={{ fontSize: "10px", fontWeight: ["clients", "reports", "settings"].includes(tab) ? 700 : 500, letterSpacing: "0.01em" }}>More</span>
          </button>
          {moreMenuOpen && (
            <div style={{
              position: "absolute", bottom: "60px", right: "8px",
              background: t.white, border: `1px solid ${t.border}`, borderRadius: "10px",
              boxShadow: "0 4px 20px rgba(0,0,0,0.12)", overflow: "hidden", minWidth: "160px",
            }}>
              {[
                { id: "clients", label: "Clients" },
                { id: "reports", label: "Reports" },
                ...
                <button key={item.id} onClick={() => { setTab(item.id); setMoreMenuOpen(false); }}
                  style={{
                    ...
                    padding: "12px 16px", background: tab === item.id ? t.surfaceAlt : "transparent",
                    border: "none", borderBottom: `1px solid ${t.borderLight}`,
                    fontSize: "14px", fontWeight: tab === item.id ? 650 : 450,
...
// DASHBOARD
// ============================================================
function Dashboard({ clients, projects, pencils, timeEntries, setTimeEntries, invoices, settings, setTab, getClient, getProject, getRate, isMobile }) {
        ...
        {upcoming.length === 0 ? <Muted>No upcoming bookings</Muted> : (
          <div style={{ display: "flex", flexDirection: "column", gap: "5px" }}>
            {upcoming.slice(0, 5).map(p => {
              const proj = p.projectId ? getProject(p.projectId) : null;
              const client = proj ? getClient(proj.clientId) : (p.clientId ? getClient(p.clientId) : null);
              ...
              return (
                <Row key={p.id}>
                  ...
                  {proj && <span style={{ fontSize: "13px", color: t.textSecondary }}>· {proj.name}</span>}
...
function Time({ timeEntries, setTimeEntries, projects, clients, pencils, settings, activeTimer, startTimer, stopTimer, elapsed, getClient, getProject, getRate, isMobile }) {
  ...
  const [mobileActionMenu, setMobileActionMenu] = useState(null); // { date, hour, entryId, y }
  ...
  return (
    <div style={isMobile ? { paddingBottom: activeProject ? "120px" : "0" } : undefined}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: isMobile ? "12px" : "18px", padding: isMobile ? "0 16px" : undefined }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          {isMobile
            ? <span style={{ fontSize: "13px", fontWeight: 650, color: t.text, minWidth: "100px", textAlign: "center" }}>{weekLabel}</span>
      ...
      {isMobile && (
        <div style={{
          ...
          padding: "2px 16px", WebkitOverflowScrolling: "touch",
        }}>
          {weekDates.map((d, i) => {
      ...
      <div style={{
        ...
        ...(isMobile ? { position: "fixed", bottom: "56px", left: 0, right: 0, zIndex: 800, borderLeft: "none", borderRight: "none", boxShadow: "0 -2px 10px rgba(0,0,0,0.06)" } : {}),
      }}>
        <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "6px" : "10px", ...(isMobile ? { overflowX: "auto", WebkitOverflowScrolling: "touch", flexWrap: "nowrap" } : { flexWrap: "wrap" }) }}>
          {!isMobile && <span style={{ fontSize: "11px", fontWeight: 700, color: t.textTertiary, textTransform: "uppercase", letterSpacing: "0.04em", minWidth: "48px" }}>Project</span>}
          {clients.length === 0 && projects.length <= 1 ? (
            ...
            <>
              ...
              {!isMobile && <div style={{ width: "1px", height: "20px", background: t.border, flexShrink: 0 }} />}
        ...
        {activeProject && (
          <div style={{ display: "flex", alignItems: "center", gap: isMobile ? "6px" : "10px", paddingLeft: isMobile ? "0" : "56px", ...(isMobile ? { overflowX: "auto", WebkitOverflowScrolling: "touch" } : {}) }}>
            ...
            />
            {!isMobile && <Btn size="sm" onClick={() => setShowBatch(true)}>Fill Range</Btn>}
            {!isMobile && <Btn size="sm" onClick={copyLastWeek}>Copy Last Week</Btn>}
            {undoStack.length > 0 && (
              <Btn v="ghost" size="sm" onClick={undo} style={{ fontSize: "11px" }}>&#8617; Undo</Btn>
            )}
            <span style={{ fontSize: "12px", color: t.textTertiary, marginLeft: "auto", whiteSpace: "nowrap" }}>{weekHours}h this week</span>
          </div>
        )}
      </div>

      {!isMobile && !activeProject && projects.length > 0 && (
      ...
      {isMobile ? (
        <div style={{ background: t.white, border: `1px solid ${t.border}`, overflow: "hidden", position: "relative" }}>
          {HOURS.map(hour => {
            ...
            const client = proj ? getClient(proj.clientId) : null;
            const pc = entry ? (projectColors[entry.projectId] || { bg: t.surfaceAlt, border: t.border, text: t.textSecondary, dot: t.textTertiary }) : null;
            const svc = entry ? SERVICE_TYPES.find(s => s.id === entry.serviceType) : null;
            const menuOpen = mobileActionMenu && mobileActionMenu.date === d && mobileActionMenu.hour === hour;

            return (
              <div key={hour} onClick={(ev) => {
                if (entry) {
                  const rect = ev.currentTarget.getBoundingClientRect();
                  setMobileActionMenu(menuOpen ? null : { date: d, hour, entryId: entry.id, projectId: entry.projectId, serviceType: entry.serviceType, y: rect.top });
                } else {
                  setMobileActionMenu(null);
                  if (activeProject) fillCell(d, hour, activeProject.projectId, activeProject.serviceType);
                }
              }}
                style={{
                  display: "flex", alignItems: "center", gap: "10px",
                ...
                <span style={{ fontSize: "12px", fontWeight: 600, color: t.textTertiary, fontFamily: "monospace", width: "32px", flexShrink: 0 }}>{fmtHour(hour)}</span>
                {entry ? (
                  <div style={{ flex: 1, display: "flex", alignItems: "center", gap: "8px", minWidth: 0 }}>
                    <div style={{ width: "8px", height: "8px", borderRadius: "2px", background: pc.dot, flexShrink: 0 }} />
                    <span style={{ fontSize: "13px", fontWeight: 600, color: pc.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{client?.name || proj?.name}</span>
                    <span style={{ fontSize: "11px", color: pc.text, opacity: 0.7, flexShrink: 0 }}>{svc?.label}</span>
                  </div>
                ) : (
                  <div style={{ flex: 1, borderBottom: activeProject ? `1px dashed ${t.border}` : "none", height: "1px" }} />
                )}
                {/* Action menu */}
                {menuOpen && (
                  <div onClick={ev => ev.stopPropagation()} style={{
                    ...
                    <button onClick={() => {
                      pushUndo(); deleteEntry(mobileActionMenu.entryId); setMobileActionMenu(null);
                    }} style={{
                      padding: "6px 12px", borderRadius: "6px", border: `1px solid rgba(197,48,48,0.2)`,
                      background: t.redBg, color: t.red, fontSize: "12px", fontWeight: 650,
                      fontFamily: "'Instrument Sans', sans-serif", cursor: "pointer",
                    }}>Remove</button>
                    <button onClick={() => {
                      const blockInfo = getBlockInfo(mobileActionMenu.date, mobileActionMenu.hour);
                      const span = blockInfo?.span || 1;
                      const startHour = blockInfo?.isStart ? mobileActionMenu.hour : mobileActionMenu.hour;
                      openNotes(mobileActionMenu.date, startHour, span, mobileActionMenu.projectId, mobileActionMenu.serviceType);
                      setMobileActionMenu(null);
                    }} style={{
                      padding: "6px 12px", borderRadius: "6px", border: `1px solid ${t.border}`,
                      background: t.white, color: t.text, fontSize: "12px", fontWeight: 650,
                      fontFamily: "'Instrument Sans', sans-serif", cursor: "pointer",
                    }}>Edit Note</button>
                  </div>
                )}
                ...
                rowCells.push(
                  <div key={`${d}-${hour}`} style={{
                    ...
                    <div
                      ...
                      onMouseEnter={ev => { ev.currentTarget.querySelectorAll('.hour-x').forEach(x => x.style.opacity = '1'); const nb = ev.currentTarget.querySelector('.block-note-btn'); if (nb) nb.style.opacity = '0.7'; }}
                      onMouseLeave={ev => { ev.currentTarget.querySelectorAll('.hour-x').forEach(x => x.style.opacity = '0'); const nb = ev.currentTarget.querySelector('.block-note-btn'); if (nb) nb.style.opacity = '0'; }}
                    >
                      {/* Label area at top */}
                      <div style={{ padding: span >= 2 ? "3px 6px 0" : "1px 6px", flexShrink: 0 }}>
                      ...
                      {/* Individual hour cells - each clickable to remove */}
                      <div style={{ flex: 1, display: "flex", flexDirection: "column" }}>
                        {Array.from({ length: span }, (_, i) => {
                          const cellHour = hour + i;
                          return (
                            <div key={cellHour}
                              onClick={() => { pushUndo(); const ce = getEntry(d, cellHour); if (ce) deleteEntry(ce.id); }}
                              style={{
                                flex: 1, cursor: "pointer", position: "relative",
                                borderTop: i > 0 ? `1px dashed ${pc.border}` : "none",
                              }}
                            >
                              <span className="hour-x" style={{
                                position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)",
                                fontSize: "10px", fontWeight: 700, color: pc.text,
                      ...
                      {/* Note button */}
                      <button className="block-note-btn"
                        onClick={(ev) => { ev.stopPropagation(); openNotes(d, hour, span, e.projectId, e.serviceType); }}
                        style={{
                          position: "absolute", bottom: "2px", right: "2px",
                          width: "15px", height: "15px", borderRadius: "50%",
                          background: "rgba(255,255,255,0.8)", border: "none",
                          fontSize: "9px", cursor: "pointer", color: pc.text,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          opacity: blockNote ? 0.5 : 0, transition: "opacity 0.12s", lineHeight: 1,
                        }}
                      >&#9998;</button>
                    </div>
                  </div>
...
function Pencils({ pencils, setPencils, projects, setProjects, clients, setClients, getClient, getProject, settings, isMobile }) {
  ...
  const formProjects = useMemo(() => {
    const filtered = form.clientId
      ? projects.filter(p => p.clientId === form.clientId && p.id !== PERSONAL_PROJECT_ID)
      : projects.filter(p => p.id !== PERSONAL_PROJECT_ID);
    return filtered;
    ...
    return (
      <div key={p.id} onClick={() => openEdit(p)} style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
          ...
          {proj && <span style={{ fontSize: "13px", color: t.textSecondary }}>&middot; {proj.name}</span>}
                ...
                {past.slice(0, 8).map(p => {
                  ...
                  return (
                    <Row key={p.id} style={{ opacity: 0.5 }}>
                      ...
                      <span style={{ fontSize: "13px", color: t.textSecondary }}>{client?.name || "—"}{proj ? ` · ${proj.name}` : ""}</span>
              ...
              return (
                <div key={iso} style={{
                  ...
                  {entries.slice(0, 3).map(e => {
                    const proj = e.projectId ? getProject(e.projectId) : null;
                    const client = proj ? getClient(proj.clientId) : (e.clientId ? getClient(e.clientId) : null);
      ...
      {showAdd && (
        <Modal title={editPencilId ? (addType === "booking" ? "Edit Booking" : "Edit Pencil") : (addType === "booking" ? "New Booking" : "New Pencil")} onClose={() => { setShowAdd(false); setEditPencilId(null); setShowNewClient(false); setShowNewProj(false); }}>
          <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            {form.clientId && (
              <Sel label="Project (optional)" value={form.projectId} onChange={v => setForm({...form, projectId: v})}
                options={[{value:"",label:"No specific project"}, ...formProjects.map(p => ({value:p.id, label:p.name}))]} />
            )}
            <div style={{ display: "flex", gap: "6px" }}>
              <Btn v="ghost" size="sm" onClick={() => setShowNewClient(true)}>+ Client</Btn>
              {form.clientId && <Btn v="ghost" size="sm" onClick={() => { setNewProj({...newProj, clientId: form.clientId}); setShowNewProj(true); }}>+ Project</Btn>}
            </div>
            {showNewClient && (
...
function Settings({ settings, setSettings, clients, projects, pencils, timeEntries, invoices, isMobile, session }) {
  ...
  return (
    <div style={{ maxWidth: isMobile ? "100%" : "520px", padding: isMobile ? "0 16px" : undefined }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
          {session && (
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                <div style={{ fontSize: "13px", fontWeight: 600, color: t.text }}>Signed in</div>
                <div style={{ fontSize: "12px", color: t.textTertiary }}>{session.user?.email}</div>
              </div>
              <Btn v="danger" onClick={() => supabase?.auth.signOut()}>Sign Out</Btn>
            </div>
          )}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div>
